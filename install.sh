...
download_binary() {
    local binary_name="motebase-bin-${PLATFORM}"
    local url="https://github.com/${REPO}/releases/download/${VERSION}/${binary_name}"

    log info "downloading $binary_name"
    if ! curl -fsSL -o "${TMP_DIR}/motebase" "$url"; then
        log error "failed to download binary"
        log info "url: $url"
        exit 1
    fi

    chmod +x "${TMP_DIR}/motebase"
}

install_binary() {
    local install_dir="${HOME}/.local/bin"
    mkdir -p "$install_dir"
    cp "${TMP_DIR}/motebase" "${install_dir}/motebase"
    log success "installed to ${install_dir}/motebase"
...
verify_install() {
    if command -v motebase &>/dev/null; then
        log success "motebase $VERSION installed"
        printf "\n%sget started:%s\n" "$bold" "$reset"
        printf "  motebase --help\n"
